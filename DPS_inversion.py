...
class DPS_Inversion():
    def __init__(self, args):
        ...
        if self.hard_data:
            ...
            for i in args.hd_cond_where:
                if i==20:
                    self.mask[...,:60,:100] = torch.diag(torch.ones(100),20)[...,:60,:100] 
                else:
                    self.mask[...,i] = 1
            
            self.yh_obs = self.mask*self.true_F.squeeze()
            self.yh_obs = self.mask*((self.true_Ip-self.ip[0])/self.ip[2]).squeeze()
    ...
    def compute_estimation_error(self, test_data):
        ...
        try: 
            ...
            for i in tqdm(range(len(t_steps)), desc= 'Error at time-step'):
                ...
                self.sigma_xhat0[i,:] = torch.quantile(mean_err_pixel.flatten(1), q=0.6, dim=1)
    ...
    def edm_sampler(self, 
        ...
        for i, (t_cur, t_next) in pbar: # 0, ..., N-1
            ...
            if args.error_x0 != 'actual_dps': #let's just skip this is not necessary
                ...
                if i < self.num_steps - 1:
                    ...
                    cond_grads, wrmse = self.condition(denoised, x_next, i-1) #get physics gradients dL/dx_next
        ...
        try:
            temp_real = torch.load(self.save_dir+'/realizations_temp.pt', weihgts_only = True, map_location='cpu')
        ...
        if self.plot_progress : self.pltprgr(denoised,cond_grads)
    ...
    def pltprgr(self,denoised,cond_grads):
        if i%1==0:
...
for error in ['dps','prop']:
    parser = argparse.ArgumentParser()
    parser.add_argument("--workdir", type=str, default= 'D:/Work/Inversion/DPS_EDM/',  help = 'C:/Users/romie/Work/Inversion/DPS_EDM/')
    parser.add_argument("--save_dir", type=str, default= 'C:/Users/romie/OneDrive - Université de Lausanne/Codes/Inversion/DPS_EDM/Save/')
    ...
    parser.add_argument("--test_models_dir", type=str, default= 'C:/Users/romie/OneDrive - Université de Lausanne/Codes/Inversion/Test_models/')
    parser.add_argument("--net_dir", type=str, default= 'C:/Users/romie/OneDrive - Université de Lausanne/Codes/Modeling/EDM_Karras/')
    ...
    parser.add_argument("--hard_data_error", default= [.1,.05], help='Sigma for [Facies, Ip], normalized between 0 and 1')
    parser.add_argument("--hd_cond_where", default= [20,50])
    parser.add_argument("--relative_data_error", default= .05)
    parser.add_argument("--absolute_data_error", default= 1)
    parser.add_argument("--error_x0", default= error, help= 'dps / prop / actual_dps: 1) the way it is theoretically proposed / our correction / how it is implemented in their DDPM implementation')
    ...
    try:
        ...
        DPS.wrmse_s = torch.load(args.save_dir+'/rmse_s.pt', weights_only=True)
        DPS.wrmse_h = torch.load(args.save_dir+'/rmse_h.pt', weights_only=True)
